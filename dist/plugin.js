...
exports.config = {
    ...
    arrowKeysAction: {
        showIf: x => x.config_tab === 'input',
        type: 'select', defaultValue: 'seek', options: { 'Seek Video (Default)': 'seek', 'Next/Prev File (HFS Navigation)': 'navigate' },
        label: "Arrow Key Behavior", helperText: "Choose between seeking the video or navigating to the next file.", frontend: true
    },
    enableScrollVolume: {
        showIf: x => x.config_tab === 'input',
            ...
            return async () => { // wait for fileSource to be available
                ...
                const mkArgs = (src, start, extra) => {
                    ...
                    if (hardware === 'custom') {
                        ...
                        // CopyTS logic - REMOVED: Browsers often struggle with non-zero start timestamps in piped MP4.
                        // We will handle time offset on the client side.
                        // args.push('-copyts');
                    }
